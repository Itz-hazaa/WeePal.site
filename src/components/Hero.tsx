...
const Hero = () => {
  ...
  // 🔥 PASTE YOUR GOOGLE APPS SCRIPT WEB APP URL HERE (get from deployment)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const message = `Name: ${formData.name}\\nPhone: ${formData.phone}\\nChild's Grade: ${formData.grade}\\nBest Time: ${formData.time}`;
    const whatsappUrl = `https://wa.me/message/WDZQSMTFKZDDE1?text=${encodeURIComponent(message)}`;
    ...
    try {
      ...
      if (result.result === "success") {
        setSubmitStatus(`✅ Saved to Sheet (Row ${result.row})!`);
        console.log("✅ Google Sheet success:", result);
      } else {
        setSubmitStatus("⚠️ WhatsApp opened (Sheet optional)");
        console.error("Sheet error:", result.error);
      }
    } catch (error) {
      setSubmitStatus("⚠️ WhatsApp opened (Sheet backup)");
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-lg border border-input bg-background text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-accent/50 transition-all disabled:bg-muted/50 disabled:cursor-not-allowed"
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFormData({ ...formData, grade: e.target.value })}
                  disabled={isSubmitting}
                  className="w-full px-4 py-3 rounded-lg border border-input bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-accent/50 transition-all disabled:bg-muted/50 disabled:cursor-not-allowed"
                >
                  <option value="">Select grade</option>
                  {[...Array(10)].map((_, i) => (
                    <option key={i + 1} value={`Grade ${i + 1}`}>
              ...
              <button
                type="submit"
                disabled={isSubmitting}
                className="w-full bg-gradient-to-r from-accent to-accent/90 text-accent-foreground py-4 rounded-xl font-bold text-lg shadow-2xl hover:shadow-orange/50 hover:scale-[1.02] transition-all duration-200 active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed disabled:shadow-none"
              >
                {isSubmitting 
                  ? "🎯 Sending to Sheet + WhatsApp..." 
                  : "🚀 Book FREE Demo →"
                }
              </button>

              {submitStatus && (
                <p className="text-xs text-center p-2 bg-green/10 text-green-600 rounded-lg font-medium mt-2 animate-fade-in">
                  {submitStatus}
                </p>
              )}

              <p className="text-xs text-center text-muted-foreground pt-2">
                🔒 Secure • No spam • WhatsApp opens automatically
              </p>
            </form>
          </div>
        </div>
